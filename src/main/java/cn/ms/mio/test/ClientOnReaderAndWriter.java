...
public class ClientOnReaderAndWriter {
	private static final Logger logger = LoggerFactory.getLogger(ClientOnReaderAndWriter.class);
            ...
            if (socketChannel.isOpen()) {
                if (aVoid == null) {
                    ...
                    while (socketChannel.read(buffer).get() != -1) {
                        ...
                        System.out.println("客户端收到响应："+decode.toString());
...
//                        limit属性依然像clear()方法一样，设置成capacity。现在Buffer准备好写数据了，但是不会覆盖未读的数据。
                        ...
                        if (r == 50) {
                            ...
                            logger.info("Client send successfully!");
                        }
                    }
                } else {
